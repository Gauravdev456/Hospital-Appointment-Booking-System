...
app.post("/Emergency" , async (req,res) =>{
    ...
    try{    
            ...
            const Eme  = new emergency({
                ...
                ambulence: req.body.ambulence,
                time: time,
                date: date,
            })

            const Emer = await Eme.save();
            speciality.find({hid : hno.hid },function(err,spl) {
                res.render('Emebooked',{list:spl})
            })
    }catch (error){
        res.status(400).send(error);
    }
...
app.get("/Emebooked", (req,res) =>{
    res.render('Emebooked');
})
